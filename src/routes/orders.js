...
router.post("/", async (req, res) => {
  try {
    ...
    if (!restaurantId || !items?.length || !email || !orderType) {
      return res
        .status(400)
        .json({ error: "restaurantId, items[], email & orderType required" });
    }

    if (orderType === "delivery" && !address) {
      return res
        .status(400)
        .json({ error: "Address is required for delivery" });
    }
...
router.get("/:id", async (req, res) => {
  try {
    const order = await Order.findById(
      req.params.id,
      "restaurantId items total email status history createdAt"
    );
